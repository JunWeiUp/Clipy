...
final class ScrollCaptureController {
    ...
    /// Guards against `onSessionDone` firing twice (startSession first-frame
    /// failure + a later user-initiated stopSession).
    private var didDeliverFinalImage = false
    ...
    func startSession() async {
        ...
        resolveTargetWindow()
        resolveTargetApp()
        // Resolve the ScreenCaptureKit display + excluded windows once (macOS 14+).
        await resolveScreenCaptureKitContext()

        // Capture first settled frame
        guard let firstFrame = await captureSettledFrame() else {
            // Session couldn't start (first frame capture failed). Still deliver
            // so the caller tears down — guarded so a later stopSession is a no-op.
            didDeliverFinalImage = true
            onSessionDone?(nil)
            return
        }
    ...
    func stopSession() {
        // Always tear down + deliver, even if the session never became active
        // (e.g. first-frame capture failed). Without this, the caller's `stop`
        // completion never fires and the overlay is stranded ("nothing happens
        // after clicking stop"). Guard against double-delivery with a flag.
        guard !didDeliverFinalImage else { return }
        didDeliverFinalImage = true
        ...
        // Deliver whatever we have so far (nil if the session produced nothing).
    ...
    /// Cached ScreenCaptureKit context: the display under the capture rect plus
    /// the SCWindow objects to exclude. Resolved once at session start so each
    /// frame grab is a single `captureImage` call. nil on macOS < 14 or if the
    /// display couldn't be resolved (falls back to CGWindowList on macOS 13).
    private var scDisplay: SCDisplay?
    private var scExcludedWindows: [SCWindow] = []
    /// Pixel size of one full-display SCK capture (cached after the first grab).
    private var scFullPixelSize: CGSize = .zero

    /// Resolve the SCK display + excluded windows for this session.
    /// Called once from `startSession` (after captureRectCG is set).
    private func resolveScreenCaptureKitContext() async {
        guard #available(macOS 14.0, *) else { return }
        guard let content = try? await SCShareableContent.excludingDesktopWindows(
            false, onScreenWindowsOnly: true) else { return }

        // Match the NSScreen to an SCDisplay via its display ID.
        let displayID = screen.deviceDescription[NSDeviceDescriptionKey("NSScreenNumber")] as? CGDirectDisplayID
        scDisplay = content.displays.first { $0.displayID == displayID } ?? content.displays.first

        // Resolve excluded window numbers to SCWindow objects.
        let excludeSet = Set(excludedWindowIDs)
        scExcludedWindows = content.windows.filter { excludeSet.contains(CGWindowID($0.windowID)) }
    }

    /// Captures the screen region. Uses ScreenCaptureKit on macOS 14+ (the only
    /// path that works on macOS 15+, where `CGWindowListCreateImage` was removed);
    /// falls back to `CGWindowListCreateImage` on macOS 13 and earlier. Returns a
    /// complete, compositor-finished snapshot cropped to `captureRectCG`.
    private func captureFrame() async -> CGImage? {
        if #available(macOS 14.0, *), let display = scDisplay {
            return await captureFrameViaSCK(display: display)
        }
        return captureFrameViaCGWindowList()
    }

    @available(macOS 14.0, *)
    private func captureFrameViaSCK(display: SCDisplay) async -> CGImage? {
        let filter = SCContentFilter(display: display, excludingWindows: scExcludedWindows)
        ...
        if scFullPixelSize == .zero {
            scFullPixelSize = CGSize(width: fullImage.width, height: fullImage.height)
        }

        // Crop the full-display capture to captureRectCG (CG coords: top-left origin).
        // The display image spans the full screen in CG space, so the rect maps 1:1
        // scaled by the pixel/logical ratio.
        let scaleX = CGFloat(fullImage.width) / screen.frame.width
        let scaleY = CGFloat(fullImage.height) / screen.frame.height
        let cropRect = CGRect(
            x: captureRectCG.origin.x * scaleX,
            y: captureRectCG.origin.y * scaleY,
            width: captureRectCG.width * scaleX,
            height: captureRectCG.height * scaleY)
        return fullImage.cropping(to: cropRect)
    }

    /// macOS 13 fallback. On macOS 15+ this returns nil (the symbol is gone),
    /// which is why the SCK path above is preferred.
    private func captureFrameViaCGWindowList() -> CGImage? {
        let excludeSet = Set(excludedWindowIDs)
        ...
        return CGWindowListCreateImage(captureRectCG, listOption, windowID, imageOption)
    }
