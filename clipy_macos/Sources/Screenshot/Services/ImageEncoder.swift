...
enum ImageEncoder {
    ...
    /// Encode an NSImage as PNG, regardless of the configured format.
    /// Used by clipy1's history ingest (which always wants paste-compatible PNG).
    ...
    /// Encode WebP via native ImageIO/CGImageDestination (org.webpmp, macOS 11+).
    /// Replaces the former Swift-WebP dependency with the system encoder so the
    /// project has no external package dependencies.
    private static func encodeWebP(bitmap: NSBitmapImageRep, quality: CGFloat) -> Data? {
        guard let cgImage = bitmap.cgImage else { return nil }
        return encodeWithCGImageDestination(cgImage: cgImage, type: "org.webpmp", lossyQuality: quality)
    }
