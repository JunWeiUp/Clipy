...
class KeystrokeOverlay: NSPanel {
    ...
    func startMonitoring() {
        ...
        // Permission gate: CGEvent.tapCreate silently returns nil without it,
        // and macOS only snapshots Input Monitoring trust at process launch —
        // granting it at runtime requires a full app restart before taps work.
        guard Self.hasInputMonitoringPermission else {
            appLog("KeystrokeOverlay: Input Monitoring not granted — key display disabled (restart app after granting)", level: .warning)
            return
        }
        ...
        appLog("KeystrokeOverlay: event tap installed (showAll=\(UserDefaults.standard.bool(forKey: "keystrokeShowAll")))", level: .info)
    }
